...
const CATEGORIES = ['All', 'food', 'drinks', 'coffee', 'health', 'retail', 'services', 'auto'];
...
export default function HomeScreen() {
  ...
  const modalPanResponder = useRef(
    PanResponder.create({
      onStartShouldSetPanResponder: () => true, // Allow touch capture on the handle
  ...
  const closeModal = () => {
    ...
    panY.setValue(0); // Reset animation for next time
  };
                ...
                <>
                  <View style={styles.modalTopRow}>
                    <TouchableOpacity onPress={() => setShowBizDetail(true)} style={styles.modalBizRow} activeOpacity={0.7}>
                      <Ionicons name="location" size={14} color="#059669" style={{ marginRight: 6 }} />
                      <Text style={styles.modalBiz}>{selectedDeal?.businessName}</Text>
                      <Text style={styles.modalBizSub}>View Location</Text>
...
const styles = StyleSheet.create({
  ...
  container: { 
    flex: 1, 
    paddingTop: 0, // Completely remove top padding
  },
  headerTitle: { 
    fontSize: 28, 
    fontWeight: '800', 
    ...
    marginBottom: 4,
    marginTop: Platform.OS === 'ios' ? 8 : 4, // Add minimal top margin
  },
