...
export default function EventsScreen() {
  ...
  const [events, setEvents] = useState<LokalaEvent[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const formattedEvents: LokalaEvent[] = (data ?? []).map((row: ChamberEventRow) => {
          const start = new Date(row.start_at);
          return {
            id: row.external_id,
            date: start.toISOString().slice(0, 10), // YYYY-MM-DD
            time: start.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }),
            businessName: 'Mid-Maine Chamber',
            title: row.title,
            description: row.description || 'No description available',
            address: row.location || 'Location TBD',
            category: 'community', // JEvents doesn't include category in iCal
          };
        });
          ...
          <View style={styles.eventsSection}>
            ...
            {loading ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySub}>Please wait while we fetch the latest events.</Text>
              </View>
